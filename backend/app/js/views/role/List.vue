                            ...
                            <div class="card-header border-0">
                                <div class="form-group my-3">
                                    <div class="input-group">
                                        <input type="text" class="form-control form-control-sm form-control-solid bg-white" v-model="search" placeholder="Search role..." v-on:keyup.enter="searchClick">
                                ...
                                <div class="d-flex align-items-center py-1 ml-auto" v-if="hasPermission('roles-create')">
                           <router-link :to='{name:"create"}' class="btn btn-sm btn-primary">Add New role
                           </router-link>
                        </div>
                              </div>
                            ...
                            <div class="card-body">
                                        ...
                                        <tr v-for="(role,key) in rolesData" :key="key">
                                            ...
                                            <td class="align-middle action-td text-center">
                                                <ul class="list-unstyled list-inline m-0">
                                                    <li class="list-inline-item mb-2" title="View" v-tooltip="tooltip.show" v-if="hasPermission('roles-view')">
                                                    ...
                                                    <li class="list-inline-item mb-2" title="Edit" v-tooltip="tooltip.edit" >
                                                        <router-link :to='{name:"Edit",params:{id:role.id}}' v-if="hasPermission('roles-edit')"><button class="btn btn-sm btn-light-warning p-0 text-center h-30px w-30px" type="button"><i class="p-0 fa fa-edit"></i></button></router-link>
                                                      
                                                    </li>
                                                    <li class="list-inline-item mb-2" title="Delete" v-tooltip="tooltip.delete" v-if="hasPermission('roles-delete')"> 
                                                        <button class="btn btn-sm btn-light-danger p-0 text-center h-30px w-30px" type="button" @click="deleteRoles(role.id)"><i class="p-0 fa fa-trash"></i></button>
                                                    </li>
                                                </ul>
                                            </td>
                                        </tr>
                                    </tbody>
                                    <tbody v-if="rolesData.length == 0">                                        <tr>
                                ...
                                <div class="col-md-12 col-12 mt-5">
                                    <div class="d-flex justify-content-end align-items-center flex-wrap">
                                        <pagination :data="roles" :limit="2" @pagination-change-page="getRoles()"></pagination>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods:{
        async getRoles(page = 1) {
            ...
            Role.index(page, this.search).then(response => {
